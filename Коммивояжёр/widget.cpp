...
//создание нескольких нескольких объектов класса Graph с разными емкостями и координатами вершин, а также обьявление массива символов, представляющих каждую вершину графа.
...
//функция обрататывающая событие нажатия на кнопку "добавить/удалить ребро"
...
//функия обратавыщая событие нажатия на кнопку "сохранить"
...
// Функция расчета длины текущего пути
int calculatePathCost(const std::vector<int>& path, const std::vector<std::vector<int>>& adjacencyMatrix)
{
    int cost = 0;
    int n = path.size();
    for (int i = 0; i < n; ++i)
    {
        int u = path[i];
        int v = path[(i + 1) % n]; // учтите зацикленность пути
        cost += adjacencyMatrix[u][v];
    }
    return cost;
}

// Функция для выполнения алгоритма ветвей и границ для задачи коммивояжера
void branchAndBoundTSP(const std::vector<std::vector<int>>& adjacencyMatrix, std::vector<int>& path, int& minCost, std::vector<int>& bestPath, std::vector<bool>& visited, int costSoFar = 0, int level = 1, int pathCost = 0, int sumOfEdges = 0)
{
    int n = adjacencyMatrix.size();

    // Базовый случай: если все вершины посещены
    if (level == n && path[level - 1] == 0)
    {
        // Проверяем, является ли текущий путь лучшим
        int currentCost = costSoFar + adjacencyMatrix[path[level - 1]][0];
    ...
    // Для каждой непосещенной вершины из текущей вершины
    for (int i = 0; i < n; ++i)
    {
        if (!visited[i] && adjacencyMatrix[path[level - 1]][i] != 0)
        {
            // Присваиваем значение текущей вершины и помечаем ее как посещенную
            path[level] = i;
            visited[i] = true;

            // Вычисляем оценку суммы ребер
            sumOfEdges += adjacencyMatrix[path[level - 1]][i];

            // Вычисляем оценку длины пути
            int currentPathCost = pathCost + adjacencyMatrix[path[level - 1]][i];

            // Если длина пути превышает текущий минимальный путь, отсекаем эту ветвь
            ...
            branchAndBoundTSP(adjacencyMatrix, path, minCost, bestPath, visited, costSoFar + adjacencyMatrix[path[level - 1]][i], level + 1, currentPathCost, sumOfEdges);
