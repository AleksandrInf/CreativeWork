...
Flight12::Flight12(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Flight12)
{
    ui->setupUi(this);
    ...
    ui->Flight_push->setMinimum(-4);
    ui->Flight_push->setMaximum(4);
}
...
void Flight12::on_Save_clicked()
{
    QString Flight = ui->Flight_push->text();
    QString F_name = ui->F_name_push->text();
    QString Price = ui->Price_push->text();
    QString Time = ui->Time_push->text();
    QDate birthDate = ui->Birth_date_push->date();
    QDate checkInDate = ui->Checkin_date_push->date();

    bool validRoomNumber = false;
    int roomNumber = Flight.toInt(&validRoomNumber);
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text))
    {
        ...
        stream << "Flight: " << Flight << endl;
        stream << "Price: " << Price << endl;
        stream << "Time: " << Time << endl;
        stream << "Book: " << birthDate.toString("dd.MM.yyyy") << endl;
        stream << "Book a day: " << checkInDate.toString("dd.MM.yyyy") << endl;
        stream << endl;
        file.close();

        // очистка
        ui->Flight_push->clear();
...
void Flight12::on_pushButton_clicked()
{
    this->close();
    emit fourthWindow();
}
void Flight12::on_pushButton_2_clicked()
{
    this->close();
    emit flightOne();
}
...
//Стоимость
void Flight12::on_pushButton_5_clicked()
{
    if((ui->rad1->isChecked()) && (ui->rad6->isChecked())) {
        QMessageBox::information(this, "Стоимость", "К оплате 60руб");
    }
    if((ui->rad1->isChecked()) && (ui->rad7->isChecked())) {
        QMessageBox::information(this, "Стоимость", "К оплате 120руб");
    }
    if((ui->rad1->isChecked()) && (ui->rad8->isChecked())) {
        QMessageBox::information(this, "Стоимость", "К оплате 170руб");
    }
    ...
    if((ui->rad2->isChecked()) && (ui->rad8->isChecked())) {
        QMessageBox::information(this, "Стоимость", "К оплате 110руб");
    }
...
//Время
void Flight12::on_pushButton_4_clicked()
{
...
//Перейти к оплате
void Flight12::on_pushButton_3_clicked()
{
    if((ui->rad1->isChecked()) && (ui->rad5->isChecked()) && (ui->rad10->isChecked())) {
        QMessageBox::information(this, "Время поезкди", "Отбытие в 10:20, Выберете конечную остановку");
    }
